...
def get_stock_age():
    ...
    """
    # columns = ['sku', 'charge_currency', 'cargo_type', 'warehouse_code', 'warehouse_id', 'warehouse_name', 'warehouse',
    #            'warehouse_stock', 'inventory_age', 'charge_total_price']
    # data = con.execute(sql)
    # df_stock_age = pd.DataFrame(data=data, columns=columns)
    ...
    sql = f"""
        ...
        FROM (
            ...
            WHERE         
                ...
                and yw.warehouse_other_type = 2   -- 筛选公共仓（非子仓\独享仓）
    ...
    for n in range(0, len(sku_list), 5000):
        ...
        sql = f"""
            with  account_list as (
                select distinct id,account_id from cj_sale_center_system_sync.cj_system_account
            ...
            FROM (
                ...
                FROM cj_sale_center_listing_sync.cj_ebay_online_listing_sales_sku 
                WHERE sku in ({sku})
            ) a 
            INNER JOIN (
                SELECT account_id,warehouse_category_id,siteid,item_id,sell_sku,seller_work_no,product_line_id,listing_status
                from cj_sale_center_listing_sync.cj_ebay_online_listing 
            ...
            LEFT JOIN (
                ...
                FROM cj_sale_center_listing_sync.cj_ebay_item_shipping
            ...
            LEFT JOIN (
                ...
                FROM domestic_warehouse_clear.cj_site_table_ebay 
                where is_open='是'
            ) c  ON b.siteid = c.site_id
            LEFT JOIN account_list d on b.account_id = d.id
            INNER JOIN (
                SELECT id, warehouse
                FROM cj_sale_center_common_sync.cj_common_big_warehouse
                WHERE warehouse_type_id in (2, 3)
            ) e ON b.warehouse_category_id = e.id

        """
        conn_ck = pd_to_ck(database='cj_sale_center_listing_sync', data_sys='调价明细历史数据')
    ...
    for n in range(0, len(sku_list), 1000):
        ...
        sql_walmart = """
                SELECT 
                    ...
                    a.sku as sku,a.seller_sku as seller_sku, toString(a.item_id) item_id, price
                FROM (
                    select * from cj_sale_center_listing_sync.cj_walmart_report 
                    where upper(publish_status)='PUBLISHED' and sku in {}
                    order by updated_unix desc limit 1 by erp_id,seller_sku
                ) a
                left join (
                SELECT erp_id, short_name, lower(site_code) as site, status
                FROM cj_sale_center_system_sync.cj_system_account
...
def get_allegro_listing_num(sku_list, df_site):
    ...
    FROM (
        ...
        FROM cj_sale_center_listing_sync.cj_allegro_listing
        WHERE 
    ...
    INNER JOIN (
        ...
        FROM cj_sale_center_system_sync.cj_system_account b
        inner join (
            select account_id,splitByString('->', oa_group_name)[-1] as group_name
            from cj_account_manage_sync.cj_allegro_account
    ...
    """
    conn_kd = pd_to_ck(database='cj_sale_center_listing_sync', data_sys='调价明细历史数据')
...
def get_temu_listing_num():
    ...
    sql = """
    with d as
    (select product_spu_id,product_sku_id,max(id) as id from cj_sale_center_listing_sync.yibai_temu_listing_crawling_log
    ...
    temu_group_bind = pd.pivot_table(df_bind_listing, values='product_sku_id', index=['sku', 'warehouse'],
                                columns=['主体账号'], aggfunc='count', fill_value=0, margins=True)
    temu_group_bind = temu_group_bind.reset_index()
    temu_group_bind = temu_group_bind.rename(columns={'All':'绑定_全团队链接条数'})
    temu_group_bind['绑定_易佰泛品链接条数'] = temu_group_bind['绑定_全团队链接条数'] - temu_group_bind['易佰精品：temu二部（茜茜子）']
    temu_group_pub = pd.pivot_table(df_temu_listing, values='product_sku_id', index=['sku', 'warehouse'],
                                columns=['主体账号'], aggfunc='count', fill_value=0, margins=True)
    temu_group_pub = temu_group_pub.reset_index()
    temu_group_pub = temu_group_pub.rename(columns={'All':'发布_全团队链接条数'})
    # temu_group_pub['发布_易佰泛品链接条数'] = temu_group_pub['发布_全团队链接条数'] - temu_group_pub['易佰精品：temu二部（茜茜子）']

    temu_group = pd.merge(temu_group_bind, temu_group_pub, how='left', on=['sku','warehouse'])

    return temu_group, df_temu
...
def get_shopee_listing_num():
    ...
    sql = f"""
        ...
        and status_online in ('NORMAL','UNLIST')
    """
    conn_ck = pd_to_ck(database='yibai_account_manage_sync', data_sys='调价明细历史数据')
    ...
    # df.to_excel('F://Desktop//yibai_shopee_listing.xlsx', index=0)
...
# df_sku_listing_new = get_sku_info()
# ##
# df_clear_sku = pd.read_excel('F://Ding_workspace//优先配库海外仓发货sku明细20241203.xlsx')
# df_clear_sku = df_clear_sku[['sku','warehouse','目的国']]
# ##
...
def main_ym():
    ...
    save_df(df, 'CJ-海外仓SKU各平台刊登情况-temp', file_type='xlsx')
    print('done!')
    # df_sku_listing_site_test.to_excel('df_sku_listing_site.xlsx', index=0)
...
def get_ym_sku():
    """ 供应商货盘sku """
    df = pd.read_excel('F://Ding_workspace//供应商货盘-刊登覆盖率统计 20250908.xlsx', dtype={'YM-SKU':str, 'YB-SKU':str})
    df_1 = df[['YM-SKU','仓库所在国家']]
...
def get_fba_fbm_order():
    ...
    where sku in {earphones_list} and created_time >= '2025-08-01'
    """
...
def get_fba_fbm_stock():
    ...
    date_id = '20250908'
    sql = f"""
    SELECT '{date_id}' `日期`,account_id ,seller_sku ,afn_fulfillable_quantity `可售库存`,sku,cost,
    `超90天库龄`
    FROM fba_inventory.amazon_fba_inventory_age_self_calculated_{date_id} a
    INNER JOIN (
    ...
    fba_stock['可售库存金额'] = fba_stock['可售库存'] * fba_stock['cost']
    print(fba_stock.info())

    fba_stock.to_excel('F://Desktop//fba_stock.xlsx', index=0)
...
if __name__ == '__main__':
    ...
    # df_account = get_temu_account()
    # df = get_allegro_listing_num('a', 'b')
