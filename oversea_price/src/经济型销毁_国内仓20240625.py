...
def get_sku_by_age():
    ...
    df['超360天库龄库存'] = (df['age'] > 360).astype('int') * df['在库库存']
    df['超300天库龄库存'] = (df['age'] > 300).astype('int') * df['在库库存']
...
def main():
    ...
    经济型销毁_国内仓清仓全平台
    筛选条件：
    1、已达到销毁价15天 & 预计可售天数大于30天
    2、已达到销毁价30天 & 预计可售天数大于15天
    1 或者 2，分段后交叉透视，统计超420/360/300库存金额。
    """
    ...
    for df_member in tqdm(list_group, total=len(list_group)):
        ...
        # 0.7*7days_sales/7+0.2*15days_sales/15+0.1*30days_sales/30 as day_sales
        ...
        sql = f"""
            SELECT sku, 30days_sales, Average_daily_sales day_sales
            FROM support_document.domestic_warehouse_clear_sku_20250804
            where sku in {tuple(df_member['sku'].values)}
        """
    ...
    df5['30days_sales'] = df5['30days_sales'].fillna(0).astype(int)
    print(df5.info())
    ...
    df[['30days_sales','30days_sales_dcm','day_sales']] = df[['30days_sales','30days_sales_dcm','day_sales']].fillna(0)
    df['esd'] = (df['在库库存']/df['day_sales']).replace(np.inf, 9999)
    # df = df[(df['30days_sales'] == 0) & (df['30days_sales_dcm'] == 0)]
    # 筛选条件
    c1 = (df['已达销毁价天数'] >= 15) & (df['esd'] > 30)
    c2 = (df['已达销毁价天数'] >= 30) & (df['esd'] > 15)
    df = df[c1 | c2]
    del df5
    ...
    df['在库库存金额'] = df['在库库存'] * df['成本']
    df['超420天库龄库存金额'] = df['超420天库龄库存'] * df['成本']
    df['超360天库龄库存金额'] = df['超360天库龄库存'] * df['成本']
    df['超300天库龄库存金额'] = df['超300天库龄库存'] * df['成本']
    df['已达销毁价天数分段'] = pd.cut(df['已达销毁价天数'],
                                 [0, 30, 45, 60, 90, 2000],
                                 labels=['A:(15,30]', 'B:(30,45]', 'C:(45,60]', 'D:(60,90]', 'E:(90,+)'])
    df['预计可售天数分段'] = pd.cut(df['esd'],
                                 [0, 30,  60, 90, 180, 10000],
                                 labels=['A:(15,30]', 'B:(30,60]', 'C:(60,90]', 'D:(90,180]', 'E:(180,+)'])
    df['预计可售天数分段'] = np.where(df['esd'] > 10000, 'E:(180,+)', df['预计可售天数分段'] )
