...
f_path1 = os.path.join(f_path, 'FBA_fulirun_newst_stock')
...
def fba_newest_stock_to_distribute_system():
    try:
        ...
        f_path_ZIP1 = os.path.join(f_path1, 'WALMART', f'{time.strftime("%Y%m%d")}', ming)
        if not os.path.exists(f_path_ZIP1):
            os.makedirs(f_path_ZIP1)
        ...
        username, password, host, port = pbm.get_ck_conf_data('调价明细历史数据')
        ck_client = CkClient(user=username, password=password, host=host, port=port,
                             db_name='support_document')
        fba_clear = ck_client.ck_execute_sql(f"show tables like 'fba_clear_seller_sku_{d}'")[0]
        if len(fba_clear)>0:
            fba_clear=fba_clear[0][0]
            # sql1 = f"""   select A.site,B.short_name,A.seller_sku,A.Current_price as "当前售价",A.sku,A.available_stock as "当前库存数量",A.Average_daily_sales as "日均销量"
            #                 from  support_document.{fba_clear} A
            #                 LEFT JOIN yibai_system_kd_sync.yibai_amazon_account B ON toUInt64(A.account_id)=toUInt64(B.id)
            #                 where A.adjustment_priority ='负利润加快动销' and A.available_stock >0 """
            sql1=f"""select A.site as site,B.short_name short_name ,A.seller_sku seller_sku,A.Current_price as "当前售价",A.sku,A.available_stock as "当前库存数量",A.Average_daily_sales as "日均销量" 
                    ,splitByString('>>', pl.path_name)[1] as linest,A.adjustment_priority as "销售状态"
                    from  support_document.{fba_clear} A
                    LEFT JOIN yibai_system_kd_sync.yibai_amazon_account B ON toUInt64(A.account_id)=toUInt64(B.id)
                    left join yb_datacenter.yb_product p on A.sku=p.sku
                    left join yb_datacenter.yb_product_description pd on
                    pd.sku = p.sku and pd.language_code = 'Chinese'
                    left join yb_datacenter.yb_product_linelist pl on
                    pl.id = toUInt64(p.product_linelist_id)
                    where (A.adjustment_priority in ('负利润加快动销' ,'正利润加快动销')
                    and A.available_stock >0)
                    or (A.adjustment_priority in ('正常')
                    and A.available_stock >= 30)"""
            df1=ck_client.ck_select_to_df(ck_sql=sql1)
            if len(df1)>0:
                print("FBA昨天的库存数据量为",len(df1))
                # excel_name = os.path.join(f_path_ZIP, f'FBA加快动销最新库存数据-{d}.xlsx')
                # excel_name1=os.path.join(f_path_ZIP1, f'FBA加快动销最新库存数据-{d}.xlsx')

                df1 = df1.reset_index(drop=True)
                df1['index'] = df1.index
                df1['index'] = df1['index'].apply(lambda m: int(m/1000000))
                for key,group in df1.groupby(['index']):
                    group.drop(['index'], axis=1, inplace=True)
                    group.to_excel(os.path.join(f_path_ZIP, f'FBA加快动销最新库存数据-{d}-{key}.xlsx'), sheet_name=f'FBA加快动销最新库存数据', index=False)
                df1=df1[df1['site']=='美国']
                df1 = df1.reset_index(drop=True)
                df1['index'] = df1.index
                df1['index'] = df1['index'].apply(lambda m: int(m / 1000000))
                for key,group in df1.groupby(['index']):
                    group.drop(['index'], axis=1, inplace=True)
                    group.to_excel(os.path.join(f_path_ZIP1, f'FBA加快动销最新库存数据-{d}-{key}.xlsx'),sheet_name=f'FBA加快动销最新库存数据', index=False)
                # exit()
                ...
                send_msg('EBAY调价LISTING问题沟通群', '动销组定时任务推送', 'FBA加快动销最新库存数据已上传至销售工作台\n明细数据可在【销售中台-->文件分发管理-->文件下载中心】下载\n请核查数据后上传')

                upload_zip(f_path1,f'WALMART',userCode='208254')
                # 发送钉钉消息
                send_msg('walmart平台调价', '动销组定时任务推送', 'FBA加快动销最新库存数据已上传至销售工作台\n明细数据可在【销售中台-->文件分发管理-->文件下载中心】下载\n请核查数据后上传')
...
if  __name__=="__main__":
    ...
    fba_newest_stock_to_distribute_system()
